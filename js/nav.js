...
var bulle_2_Ouvert_Right = "384px";
var bulle_2_Ferme_Right = "374px";
...
var lien2_4_Ouvert_Right = "282px";
...
$(document).ready(function(){
	...
	$('.lien2-1, .lien2-2, .lien2-3, .lien2-4').click(function(evt){
		evt.stopPropagation();
		...
		$('.lien2-4').mouseout().stop().animate({right: sousLien_Ferme_Right}, 50);
	});
    ...
    if($(window).width() > 1366){
		...
		$('.nav .lien2, .lien2-1, .lien2-2, .lien2-3, .lien2-4, .sous-liens').hover(
            function(){
        ...
        $('.lien2, .lien2-1, .lien2-2, .lien2-3, .lien2-4, .sous-liens').hover(
            function(){
                ...
                $('.lien2-4').stop().animate({right: lien2_4_Ouvert_Right}, 50, 'swing');
            },
            function(){
                ...
                $('.lien2-4').stop().animate({right: sousLien_Ferme_Right}, 50);
            }
        );
	}
		...
		function(evt){
			...
			if(toutOuvert){
				...
				if($('.lien2-0-mobile').css('right') === sousLien_Ferme_Right_Mobile){
					...
					$('.lien2-4-mobile').animate({right: lien2_4_Ouvert_Right_Mobile}, 300);
				}
					...
					$('.lien2-4-mobile').animate({right: sousLien_Ferme_Right_Mobile}, 300);
				}
				if($('.bulle_2-0-mobile').css('opacity') === "0"){
					...
					$('.bulle_2-4-mobile').animate({opacity: 1}, 300);
				}
					...
					$('.bulle_2-4-mobile').animate({opacity: 0}, 300);
				}
	...
	if(window.location.href.indexOf("portfolio.php") != -1){
		$(".sous-liens a, .sous-liens-mobile a").addClass("pas-effet");
		
		$(".sous-liens a, .sous-liens-mobile a").click(function(evt){
			evt.preventDefault();
			
			var pos = $(this).attr("href").indexOf("section") + 7
			var num = $(this).attr("href").charAt(pos);
...
function ouvrirMobile(){
	...
	$('.nav-mobile').removeClass("fermee", 600, "easeOutElastic", function(){
		$('.bulle_1-mobile, .bulle_2-mobile, .bulle_2-0-mobile, .bulle_2-1-mobile, .bulle_2-2-mobile, .bulle_2-3-mobile, .bulle_2-4-mobile, .bulle_3-mobile, .bulle_4-mobile, .bulle_5-mobile').show();
		
		$('.lien2-0-mobile, .lien2-1-mobile, .lien2-2-mobile, .lien2-3-mobile, .lien2-4-mobile').show();
...
function fermerMobile(){
	...
	$('.nav-mobile').addClass("fermee", 600, "easeOutBounce", function(){
		$(".bulle_1-mobile, .bulle_2-mobile, .bulle_3-mobile, .bulle_4-mobile, .bulle_5-mobile").each(function(index){
			...
			$('.bulle_1-mobile, .bulle_2-mobile, .bulle_2-0-mobile, .bulle_2-1-mobile, .bulle_2-2-mobile, .bulle_2-3-mobile, .bulle_2-4-mobile, .bulle_3-mobile, .bulle_4-mobile, .bulle_5-mobile').hide(function(){
				toutOuvert = false;
			});
		});
	});
...
function fermerSousMenuMobile(){
	...
	if($('.nav-mobile').hasClass("sous-menu-ouvert")){
		...
		$('.lien2-4-mobile').animate({right: sousLien_Ferme_Right_Mobile}, 100, function(){
			$('.lien2-4-mobile').hide();
		});
		...
		$('.bulle_2-4-mobile').css('opacity', 0);
